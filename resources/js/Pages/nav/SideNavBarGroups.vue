            ...
            <div>
                ...
                <div v-if="localGroups[index].expandable && localGroups[index].expand" class="pl-4">
                    <div
                        draggable="true"
                        v-bind:key="user.id"
                        v-for="user in localGroups[index].users"
                        class="text-left flex items-center text-base font-normal text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 p-2"
                        v-on:dragstart="onUserDragStart($event, localGroups[index], user)"
                        v-on:click="openUserPage(user)"
                    >
                        <img alt="Avatar" class="mr-3 cursor-pointer h-4 sm:h-4 rounded-full"
                             :src="require('~/images/avatar-svgrepo-com.svg')">
                        {{ user.name }}
                    </div>
                    <div class="text-left">
                        <side-nav-bar-groups
        ...
        groups: {
            ...
            handler() {
                if (this.groups.length > 0) {
                    this.localGroups = this.groups.map(g => ({
                        expand: false, expandable: (g.groups && g.groups.length > 0)
